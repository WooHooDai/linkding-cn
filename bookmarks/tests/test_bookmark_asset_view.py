...
class BookmarkAssetViewTestCase(TestCase, BookmarkFactoryMixin):
    ...
    def test_rename_asset(self):
        ...
        # 正常重命名
        self.client.post(
            url, {"rename_asset": asset.id, "new_display_name": "新名字.txt"}
        )
        asset.refresh_from_db()
        self.assertEqual(asset.display_name, "新名字.txt")
        # 空字符串不应修改
        self.client.post(url, {"rename_asset": asset.id, "new_display_name": "   "})
        asset.refresh_from_db()
        self.assertEqual(asset.display_name, "新名字.txt")
        # 非本人无权限
        other_user = self.setup_user()
        ...
        self.client.post(
            url, {"rename_asset": other_asset.id, "new_display_name": "非法.txt"}
        )
