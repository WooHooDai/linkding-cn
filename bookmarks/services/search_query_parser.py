            ...
            if self.query.startswith(prefix, self.position):
                ...
                if content is None:
                    raise SearchQueryParseError(
                        f"Expected RPAREN, got EOF", self.position
                    )
...
@dataclass
class FieldTermExpression(SearchExpression):
    field: str
    term: str
...
class SearchQueryParser:
    ...
    def parse_primary_expression(self) -> SearchExpression:
        ...
        elif self.current_token.type == TokenType.FIELD_TERM:
            field_name, term = self.current_token.value
            self.advance()
            return FieldTermExpression(field_name, term)
...
def _is_simple_expression(expr: SearchExpression) -> bool:
    ...
    return isinstance(
        expr,
        (TermExpression, TagExpression, SpecialKeywordExpression, FieldTermExpression),
    )
